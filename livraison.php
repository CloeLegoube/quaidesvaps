	...
	if(isset($_POST['payer'])) // Est-ce que l'internaute a cliqué sur payer ?
	{
		...
		if(!utilisateur_est_connecte())
		{
				...
				if(isset($_POST['conditions_generales'])){
						...
						for($i = 0; $i < count($_SESSION['panier']['id_produit']); $i++)
						{
							...
							if(isset($_SESSION['promotion']['code_promo'][$j]) && $_SESSION['promotion']['code_promo'][$j] === $_SESSION['panier']['code_promo'][$i]){
								...
								if(!empty($_SESSION['panier']['prix_promo'][$i])){
									...
									execute_requete
									("INSERT INTO details_commande (id_details_commande, id_commande, id_produit, quantite, prix_promo, code_promo, reduction) 
									VALUES 
									('', $id_commande, ".$_SESSION['panier']['id_produit'][$i].", ".$_SESSION['panier']['quantite'][$i].", ".$_SESSION['panier']['prix_promo'][$i].", '".$_SESSION['panier']['code_promo'][$i]."',".$_SESSION['panier']['reduction'][$i]." )  	");
								...
								("INSERT INTO details_commande (id_details_commande, id_commande, id_produit, quantite, prix_promo, code_promo, reduction) 
								VALUES 
								('', $id_commande, ".$_SESSION['panier']['id_produit'][$i].", ".$_SESSION['panier']['quantite'][$i].", 0,0,0) 	");
								
							}	
								
								//-------------------------3ème requete update dans produit pour mettre à jour le stock				
								execute_requete("UPDATE produit 
					...
					$mail = 'cloe.legoube@gmail.com'; //$_SESSION['utilisateur']['email']; // Déclaration de l'adresse de destination.'cloe.legoube@gmail.com' pour TEST
						...
						$message_html = '<html>
										...
										<font face="verdana"><font color="white"><font size="5"><p align="center"><font color="red"><u>Confirmation de votre commande</u></p></font></font>
										<font size="3"><p>'.$contenu.'</p>
										<p align="left">Visitez notre site quaidesvaps.fr, pour voir nos promotions sur les nouveaux produits</p>
										</body>
						...
						//=====Définition du sujet.
						$sujet = "QUAI DES VAPS : Confirmation de votre commande";
						//=========
...
/*								mail('cloe.legoube@gmail.com', 'Confirmation de commande', 'Merci, votre numéro de suivi est : '.$id_commande, 'From:contact@quaidesvaps.fr');
								Ici on le met en commentaire car en local cela ne fonctionnera pas */
								...
								$msg .= '<div id="msg">
											<p class="vert">Votre commande est validée. Un email de confirmation vous a été envoyé</p>
										</div>';
