...
<!-- ******* BOITE 1 : PROMOTIONS ******* -->
					<div class="block">
						<div class="titre_h2 largeur"><h2>AVIS CLIENTS</h2></div>
						<a class="lien_stat" href="?action=note#resultat"><div class="stat stat1 <?php if(isset($_GET['action']) && $_GET['action']== "note") echo 'result1'?>">
								<p>Top 8</p>
								<p>des produits les mieux notés</p>
...
<!-- ******* BOITE 3 : CARTE DE FIDELITE ******* -->
					<div class="block">
						<div class="titre_h2 largeur"><h2>VOLUME DE VENTES</h2></div>
						
							<a class="lien_stat" href="?action=quantite#resultat"><div class="stat stat3 <?php if(isset($_GET['action']) && $_GET['action']== "quantite") echo 'result3'?>">
								<p>Top 8</p>
								<p>des membres qui achetent le plus</p>
...
//**********************************************************************************************************	
//                         Top 5 des membres qui achetent le plus (en terme de quantité)
//**********************************************************************************************************	
...
if(isset($_GET['action']) && $_GET['action']== "quantite")
	{		
		...
		$quantite = execute_requete("SELECT *, COUNT(prenom) AS membre
			FROM commande c, membre m
			WHERE c.id_membre = m.id_membre
			GROUP BY c.id_membre
			ORDER BY membre DESC LIMIT 0,8" );
			$i=1;

			echo '<div id="resultat"  class="titre_h2 largeur_article clear"><h2>Top 5 des membres qui achètent le plus</h2></div>';
					
			while($produit = $quantite->fetch_assoc()){
				
				$compte = round($produit['membre'],2);
?>					 

					<div class="produit">
					<h3 class="result3">N° <?php echo $i++ ?> du classement</h3>
<?php
				echo "
						<div class='offre_produit margin_r35'>
						<h4>Nbre de commandes : ".$compte."</h4>
